...
# # # Minimum gap between requests to stay under SpiceJet's WAF rate limits.
# # DELAY_BETWEEN_REPLAY_CALLS = 12
# # DELAY_BETWEEN_ROUTES = 15
# # CONSECUTIVE_403_BACKOFF = 60  # cooldown pause if the WAF starts rejecting everything
# # MAX_CONSECUTIVE_FAILURES = 5  # abort route after this many empty/spinner captures in a row

# # OUTPUT_DIR = Path("spicedata")
# # OUTPUT_DIR.mkdir(exist_ok=True)
# # CONFIG_OUTPUT_PATH = Path(__file__).with_name("spicejet_top_24_routes.json")

# # # IMPORTANT: a bot-labelled UA ("VAYUSETU-Bot") gets fingerprinted and
# # # silently served an infinite loading-spinner challenge page instead of
# # # a clean 403 -- status still reads 200 but no API traffic ever fires.
# # # Use a real desktop Chrome UA string instead.
# # USER_AGENT = (
...
# #     # Check against a generic browser UA token ("*") rather than a bot
# #     # name -- robots.txt may have an entirely separate (stricter) block
# #     # for named bots that doesn't reflect what a real browser is allowed.
...
# # # Same 24 priority routes used for the Akasa dataset, kept identical so
# # # both airlines' records line up on route_id for the airfare index.
# # PRIORITY_ROUTES = [
...
# #     """
# #     Scrapes a single origin/dest/date using a BRAND NEW browser context
# #     (fresh cookies, storage, and sensor-token state) for every request.

# #     SpiceJet's WAF appears to fingerprint by session: the first search on a
# #     fresh context looks like a normal user landing on the page and the
# #     site's own JS fires availability + lowfare normally. Reusing the same
# #     page/context for multiple sequential searches gets treated as a bot
# #     repeat-querying the same session -- the page still returns HTTP 200,
# #     but instead of firing the real API calls it serves an endless loading
# #     spinner (a soft challenge page) that never resolves.

# #     A fresh context per date costs more time/memory than reusing one page,
# #     but is what actually gets past the WAF reliably.
# #     """
# #     captured = {"availability": None, "lowfare": None}

# #     async def handle_response(response, captured=captured):
# #         path = urlparse(response.url).path.lower()
# #         if response.status != 200:
# #             return
# #         if path.endswith("/api/v3/search/availability") and captured["availability"] is None:
# #             try:
...
# #         # Wait for either the API calls to land, or bail out early once
# #         # it's clear we're just watching a stuck loading spinner.
# #         deadline = asyncio.get_event_loop().time() + 15
# #         stuck_spinner_checked = False
...
# #             # Try to tell a genuine "no flights on this route/date" page
# #             # apart from a stuck challenge/spinner page, for easier
# #             # debugging later.
# #             try:
...
# #     """
# #     Runs scrape_single_date() for each date in travel_dates, each on its
# #     own fresh browser context, with a delay between calls to respect the
# #     WAF's rate limits. Aborts the remaining dates for this route early if
# #     too many consecutive failures suggest we've been blocked outright.
# #     """
# #     results = {}
...
# #         if outcome == "ok" or (captured["availability"] or captured["lowfare"]):
# #             consecutive_failures = 0
# #         else:
...
# #                         adv: int, route_id: str, total_passengers: int):
# #     records = []
...
# #         # One browser process is reused across the whole run (cheap), but
# #         # every individual date/search gets its own fresh context (see
# #         # scrape_single_date) so the WAF sees a "new visitor" each time
# #         # instead of one session hammering the search endpoint repeatedly.
...
# #             window_results = await scrape_route(origin, dest, date_list, browser, headless=headless)

# #             for adv, travel_date in windows:
# #                 count += 1
# #                 entry = window_results.get(travel_date, {})
# #                 normalized_all.extend(
# #                     normalize_response(entry, origin, dest, travel_date, adv, route_id, pax)
# #                 )
# #                 status = "OK" if entry.get("availability") or entry.get("lowfare") else "--"
...
# #         recs = normalize_response(captured, "BOM", "DEL", "2026-09-04", 7, "MUMBAI-DELHI", 4029444)
...
# # How many times to re-attempt a single date (fresh context each time)
# # before giving up and recording it as blocked. Each retry waits longer
# # than the last (RETRY_BACKOFF_BASE * attempt_number seconds).
...
#         if captured["availability"] is None and captured["lowfare"] is None:
#             # Distinguish three cases so retries aren't wasted on a route
#             # that genuinely has no service:
#             #   1. genuine "no flights found" text visible -> real result,
#             #      don't retry
#             #   2. a stuck loading spinner still visible -> WAF challenge,
#             #      worth retrying
#             #   3. neither -> unknown failure, worth retrying
#             try:
...
#     """
#     Keeps re-attempting a single date (fresh context each try, growing
#     backoff) until it gets real data, a confirmed genuine "no flights"
#     result, or runs out of retries. This is what makes every output
#     record end up with real fields instead of nulls from a one-shot
#     WAF block -- most blocks clear after a short cooldown + fresh
#     session, so retrying is usually enough.
#     """
...
#     return {"availability": None, "lowfare": None}, "blocked_after_retries"


# async def scrape_route(origin: str, dest: str, travel_dates: list, browser, headless: bool = True):
#     """
#     Runs scrape_date_with_retries() for each date in travel_dates, each on
#     its own fresh browser context(s), with a delay between calls to
#     respect the WAF's rate limits. Aborts the remaining dates for this
#     route early (with a long cooldown) if too many dates in a row still
#     fail even after their own internal retries -- that signals the WAF is
#     blocking this session/IP outright rather than just this one request.
#     """
#     results = {}
#     outcomes = {}
#     consecutive_failures = 0
...
#     if not records:
#         # Distinguish a genuinely empty route/date from a scrape that
#         # never got real data at all, so downstream consumers know which
#         # rows to trust vs. which to re-collect later.
